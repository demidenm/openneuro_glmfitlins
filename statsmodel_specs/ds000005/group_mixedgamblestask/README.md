...
For the 16 subjects, whole-brain, mass univariate analyses were performed using a general linear model (GLM). The 5 regressors of interest (out of 6 total regressors) were convolved with a spm hemodynamic response function (see Section 2.3 for list). Of the convolved regressors, 3 were parametrically modulated regressors in the model (see Section 2.4 for list). The design matrix (see example in Section 4.1) included both the convolved and parametrically modulated regressors of interest and 30 nuisance regressors to account for physiological noise and motion-related artifacts (see Section 2.4 for full list).
...
trial_type.1, parametric loss, parametric gain, distance from indifference, rt_reg.1, intercept

### 2.3 Convolved Regressors
trial_type.1, parametric loss, parametric gain, distance from indifference, rt_reg.1

### 2.4 Parametrically Modulated Regressors*
parametric loss, parametric gain, distance from indifference
