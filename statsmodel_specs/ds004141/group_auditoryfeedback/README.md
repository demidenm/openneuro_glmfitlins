# ds004141: auditoryfeedback Task Analysis Report

The size of the Fitlins Derivatives for ds004141 auditoryfeedback is 147M with 128 files.

Dataset- and task-relevant citations may be found in the papers: [Paper 1](https://doi.org/10.1016/j.neuroscience.2019.02.024).
...
Each voxel's timeseries (Y) was regressed onto the resulting design matrix (Xβ), which included the regressors and an intercept term . As illustrated in Section 3, 1 linear contrasts were computed
...
trial_type.Alternate, trial_type.Happy, trial_type.Neutral, trial_type.Sad, intercept

### 2.3 Convolved Regressors
trial_type.Alternate, trial_type.Happy, trial_type.Neutral, trial_type.Sad
...
- **upregfacesvdownregneut**: 0.333*`trial_type.Happy` + 0.333*`trial_type.Sad` + 0.333*`trial_type.Alternate` - 1*`trial_type.Neutral`


# 4 Figures

## 4.1 Design Matrix
![Design Matrix](./files/ds004141_task-auditoryfeedback_design-matrix.svg)

The example design matrix illustrates the model used in the statistical analyses for this task (Note: if motion outliers are included, the number of these will vary between subjects). Each column represents a regressor (of interest or not of interest, based on the above), and each row represents a time point in the BOLD timeseries. The colored patterns show how different experimental conditions are modeled across the scan duration (HRF model).

## 4.2 Contrast Weights
![Contrast Weight](./files/ds004141_task-auditoryfeedback_contrast-matrix.svg)

The contrast maps represent the weights used to model brain activity.

## 4.3 Variance Inflation Factor (VIF)
![VIF Distribution](./files/ds004141_task-auditoryfeedback_vif-boxplot.png)
...
![R Square](files/ds004141_task-auditoryfeedback_rsquare-mean.png)
...
The subjects flagged for auditoryfeedback are:
sub-02_ses-01_run-01, sub-10_ses-01_run-01

The distribution for subjects and runs in auditoryfeedback are below. 

![Dice](files/ds004141_task-auditoryfeedback_hist-dicesimilarity.png)
![Voxels Out](files/ds004141_task-auditoryfeedback_hist-voxoutmask.png)

## 5 Statistical Maps

### upregfacesvdownregneut

#### ses-01
![upregfacesvdownregneut ses-01 Map](files/ds004141_task-auditoryfeedback_ses-01_contrast-upregfacesvdownregneut_map.png)
